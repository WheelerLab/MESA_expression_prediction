...
p <- ggplot(sig_hits, aes(x = pheno, y = sig, fill=pheno)) + coord_flip()+ geom_bar(stat = "identity") +
  labs( x="Phenotype", y = "Number of Significant Genes")+theme(axis.text.y = element_text(color = "grey20", size = 10, hjust = .5, vjust = .5, face = "plain")) + facet_wrap(~mod) + scale_fill_manual(values = colorsss) + 
...
print(sig_hits)
p1 <- ggplot(sig_hits, aes(x=pheno, y = sig, fill=pheno)) + coord_flip() + geom_bar(stat='identity')+labs(x="Phenotype", y = "Number of Significant Genes")+theme(axis.text.y = element_text(color = "grey20", size = 10, hjust = .5, vjust = .5, face = "plain")) + facet_wrap(~mod) +theme_minimal() + theme(legend.position = "none") + scale_fill_manual(values = colorsss) 
p1 
ggsave('/home/egeoffroy/Summary_Statistics/Summary_Stats/Wojcik/Total_MESA_Wojcik_inordermax.png', device = 'png', p1, width = 4, height = 4)
